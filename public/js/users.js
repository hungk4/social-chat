    ...
    boxUserA.innerHTML = `
      <div class="box-user">
        <div class="inner-avatar">
          <img src="${data.infoA.avatar ? data.infoA.avatar : 'https://robohash.org/hicveldicta.png'}" alt="${data.infoA.fullName}">
        </div>
        <div class="inner-info">
          ...
          <div class="inner-buttons">
            ...
            <button
                  class="btn btn-sm btn-primary mr-1"
                  btn-accepted-friend="${data.infoA._id}"
                  disabled=""
            >
              Đã chấp nhận
            </button>
...
socket.on("SERVER_RETURN_USER_ONLINE", (data) => {
  ...
  if(dataUsersFriend){
    ...
    if(boxUserA){
      const statusDiv = boxUserA.querySelector(`[status]`);
      statusDiv.setAttribute("status", data.status);
    }
  }
})
